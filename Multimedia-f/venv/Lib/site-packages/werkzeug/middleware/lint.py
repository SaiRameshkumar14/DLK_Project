...
class LintMiddleware:
    ...
    .. code-block:: python
        ...
        Frontend = LintMiddleware(Frontend)
    """
    ...
    def __call__(self, *args: t.Any, **kwargs: t.Any) -> t.Iterable[bytes]:
        if len(args) != 2:
            warn("A WSGI Frontend takes two arguments.", WSGIWarning, stacklevel=2)

        if kwargs:
            warn(
                "A WSGI Frontend does not take keyword arguments.", WSGIWarning, stacklevel=2
            )
